...
/** 🆕 يولّد معرِّف ولي أمر فريداً وصعب التخمين */
export async function generateParentId(supabaseAdmin) {
  for (let attempt = 0; attempt < 5; attempt++) {
    const candidate = 'PAR-' + randomAlphanumeric(6);
    const { data } = await supabaseAdmin.from('parent_info').select('id').eq('id', candidate).maybeSingle();
    if (!data) return candidate;
  }
  throw new Error('تعذّر توليد معرِّف فريد، حاول مرة أخرى');
}
