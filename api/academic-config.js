...
// 🆕 خارطة أنواع الأسبوع — تُستخدَم للعرض (شارة ملوَّنة) وبنموذج التعديل
...
function renderTermsList() {
  ...
  area.innerHTML = sorted.map((t) => `
    <div class="card ${isAdmin && t.is_visible === false ? 'term-card-hidden' : ''}" style="margin-bottom:14px">
        ...
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          ${isAdmin ? `
            <button type="button" class="term-visibility-toggle ${t.is_visible === false ? 'is-hidden' : 'is-visible'}" data-toggle-visibility="${t.id}" data-current-visible="${t.is_visible !== false}">
              ${t.is_visible === false ? 'مخفي — إظهار للجميع' : 'ظاهر لكل الموظفين'}
            </button>
  ...
  if (isAdmin) {
    ...
    area.querySelectorAll('[data-toggle-visibility]').forEach((btn) => {
      btn.addEventListener('click', () => toggleTermVisibility(btn.getAttribute('data-toggle-visibility'), btn.getAttribute('data-current-visible') === 'true'));
    });
  }
}

async function toggleTermVisibility(termId, currentlyVisible) {
  try {
    await apiCall('academic-config', { method: 'POST', body: { action: 'toggleTermVisibility', id: termId, isVisible: !currentlyVisible } });
    showToast(currentlyVisible ? 'تم إخفاء الفصل عن باقي الموظفين' : 'تم إظهار الفصل لكل الموظفين', 'success');
    APP.allAcademicTerms = await apiCall('academic-config', { method: 'POST', body: { action: 'listTerms' } });
    renderTermsList();
  } catch (e) { showToast(e.message, 'error'); }
}
      ...
      ${weeks.map((w) => {
        ...
        <div class="person-card-row" style="padding:8px 10px;background:var(--surface);border-radius:8px">
          ...
          <div style="display:flex;align-items:center;gap:10px">
            ...
            ${isAdmin ? `<button type="button" class="btn-icon-edit" data-edit-week="${w.id}" title="تخصيص الأسبوع">${ICONS.edit()}</button>` : ''}
          </div>
        </div>`;
      }).join('')}
    </div>`;
  ...
  if (isAdmin) {
    area.querySelectorAll('[data-edit-week]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        if (week) openWeekEditModal(week, termId);
      });
    });
  }
}

/** 🆕 نافذة تخصيص أسبوع دراسي — النوع (دراسي/إجازة/اختبار شهري/اختبار نهائي)
 * + التسمية الحرة (تُستخدَم أيضاً لاسم الإجازة، مثل "إجازة مطر" أو "اليوم الوطني") */
function openWeekEditModal(week, termId) {
  const overlay = document.createElement('div');
  overlay.className = 'modal-overlay show';
  ...
  overlay.innerHTML = `
    <div class="modal-card" style="max-width:420px">
      <div class="modal-header">
        <h3>تخصيص الأسبوع ${week.week_number}</h3>
        <button type="button" class="modal-close-btn" id="weekEditCloseBtn">${ICONS.close()}</button>
      </div>
      <div class="modal-body">
        <div class="field"><label>نوع الأسبوع</label>
          <select id="weekEdit_type">
            ${typeOptions.map((t) => `<option value="${escapeHtml(t)}" ${t === (week.week_type || 'دراسي') ? 'selected' : ''}>${escapeHtml(t)}</option>`).join('')}
          </select>
        </div>
        <div class="field"><label>التسمية</label>
          <input type="text" id="weekEdit_label" value="${escapeHtml(week.label || '')}" placeholder="مثال: الأسبوع ${week.week_number}، أو اسم الإجازة لو كان النوع إجازة">
        </div>
        ...
        <button type="button" id="weekEditSaveBtn" style="width:100%;margin-top:6px">حفظ</button>
      </div>
    </div>`;
  document.body.appendChild(overlay);
  ...
  const close = () => overlay.remove();
  overlay.addEventListener('click', (e) => { if (e.target === overlay) close(); });
  document.getElementById('weekEditCloseBtn').addEventListener('click', close);

  document.getElementById('weekEditSaveBtn').addEventListener('click', async () => {
    const label = document.getElementById('weekEdit_label').value.trim();
    const weekType = document.getElementById('weekEdit_type').value;
    if (!label) { showToast('التسمية مطلوبة', 'error'); return; }
    const btn = document.getElementById('weekEditSaveBtn');
    btn.disabled = true; btn.textContent = 'جارِ الحفظ...';
    try {
      await apiCall('academic-config', { method: 'POST', body: { action: 'updateWeek', id: week.id, label, weekType } });
      showToast('تم تحديث الأسبوع بنجاح', 'success');
      close();
      const weeks = await apiCall('academic-config', { method: 'POST', body: { action: 'listWeeksForTerm', termId } });
      renderWeeksTable(termId, weeks);
